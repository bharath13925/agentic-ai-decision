/**
 * RagChat.jsx — AgenticIQ AI Chat Widget v2.0
 *
 * Floating chat panel powered by FAISS + Groq API RAG pipeline.
 * Retrieves relevant context from the agent pipeline outputs
 * and generates grounded answers via Groq (llama-3.3-70b-versatile).
 * Set GROQ_API_KEY in python-microservice/.env to enable LLM answers.
 *
 * Props:
 *   projectId   {string}   — Required. Used to scope the FAISS context store.
...
// VITE_API_URL from frontend/.env
// Local:  http://localhost:5000/api
// Vercel: https://api-agentiq.onrender.com/api
const API = import.meta.env.VITE_API_URL || "http://localhost:5000/api";

/* ── Design tokens matching AgenticIQ theme ── */
const C = {
  violet: "#7C5CFF",
  cyan:   "#00E0FF",
  pink:   "#FF4FD8",
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        height:          28,
        borderRadius:    8,
        background:      isUser ? `${C.violet}30` : `${C.cyan}20`,
        border:          `1px solid ${isUser ? C.violet : C.cyan}40`,
        display:         "flex",
        alignItems:      "center",
        justifyContent:  "center",
        flexShrink:      0,
        fontSize:        11,
        color:           isUser ? C.violet : C.cyan,
        fontWeight:      700,
      ...
      <div style={{ maxWidth: "80%", minWidth: 40 }}>
        <div style={{
          background:   isUser
            ? `linear-gradient(135deg, ${C.violet}25, ${C.violet}10)`
            : `${C.card}`,
          border:       `1px solid ${isUser ? `${C.violet}30` : C.border}`,
            ...
            <div style={{ paddingTop: 6, display: "flex", flexDirection: "column", gap: 4 }}>
              {sources.map((src, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ color: C.cyan, marginRight: 6 }}>
...
export default function RagChat({ projectId, agentResult, className = "" }) {
  ...
  const handleStoreContext = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        await fetchStats();
        addSystemMessage(
          `✅ Context refreshed — ${data.stored?.length || 0} analysis chunks loaded into memory.`
        );
      } else {
  ...
  const handleClearContext = async () => {
    if (!confirm("Clear the AI context for this project? You can re-store it anytime.")) return;
    try {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role:    "assistant",
        content: "Failed to get a response. Is the Python microservice running?",
        sources: [],
        time:    new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      }]);
    }
      ...
      >
        <motion.button
          ...
          style={{
            ...
            color:        "#fff",
            position:     "relative",
            boxShadow:    `0 8px 32px ${C.violet}50`,
          }}
          animate={{
            boxShadow: [
              `0 8px 32px ${C.violet}50`,
              `0 8px 32px ${C.cyan}50`,
              `0 8px 32px ${C.violet}50`,
            ],
          }}
        ...
        >
          ...
          <motion.div
            style={{
              ...
              height:       12,
              borderRadius: "50%",
              background:   hasContext ? C.green : C.orange,
        ...
        {isOpen && (
          <motion.div
            ...
            style={{
              ...
              background:   "linear-gradient(145deg, #0E0E18, #0B0B12)",
              border:       `1px solid rgba(255,255,255,0.1)`,
              borderRadius: 24,
              display:      "flex",
              flexDirection:"column",
              overflow:     "hidden",
              boxShadow:    `0 24px 80px rgba(0,0,0,0.6), 0 0 0 1px ${C.violet}20`,
            }}
          >
            {/* Top accent bar */}
            <div style={{
              height:     2,
              background: `linear-gradient(90deg, ${C.violet}, ${C.pink}, ${C.cyan})`,
            }} />
            ...
            <div style={{
              ...
              <motion.div
                ...
                style={{
                  ...
                  height:     36,
                  borderRadius:10,
                  background: `linear-gradient(135deg, ${C.violet}30, ${C.cyan}20)`,
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: C.muted, fontSize: 10, fontFamily: "monospace" }}>
                  {statsLoading
                    ? "Checking context…"
                    : hasContext
                    ? `${stats?.total_docs || 0} analysis chunks · RAG enabled`
                    : "No context — store analysis first"
                  }
                </div>
              </div>
            ...
            {!hasContext && !storeLoading && (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ color: C.orange, fontSize: 11, fontWeight: 600, margin: 0 }}>
                    No analysis context stored
                  </p>
                  <p style={{ color: C.muted, fontSize: 10, margin: "2px 0 0" }}>
                    Click refresh to load context from the agent pipeline.
                  </p>
                </div>
                <motion.button
            ...
            {hasContext && stats && (
              <div style={{
                ...
                {[
                  ...
                  { label: stats.embedding_model || "embedding", color: C.violet },
                  { label: stats.faiss_available ? "FAISS" : "cosine", color: C.green },
                  {
                    label: stats.groq_available ? "Groq LLM ready" : "LLM offline",
                    color: stats.groq_available ? C.green : C.orange,
                  },
                ].map(({ label, color }) => (
            ...
            <div style={{
              ...
              padding:    "14px 14px 0",
              scrollbarWidth:"thin",
              scrollbarColor:`${C.violet}30 transparent`,
            }}>
              {/* Empty state */}
              {messages.length === 0 && (
                <div style={{ textAlign: "center", padding: "20px 0" }}>
                  ...
                  <p style={{ color: C.muted, fontSize: 13, fontWeight: 600, margin: "0 0 4px" }}>
                    Ask about your analysis
                  </p>
                  <p style={{ color: `${C.muted}80`, fontSize: 11, margin: "0 0 16px" }}>
                    Get grounded answers from your ML pipeline results
                  </p>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {SUGGESTED_QUESTIONS.slice(0, 4).map((q, i) => (
                      <motion.button
                        ...
                        whileHover={{ x: 3, borderColor: `${C.violet}60` }}
              ...
              {messages.map((msg, i) => {
                if (msg.role === "system") {
                  return (
                    ...
                    >
                      <FiCheck size={9} style={{ marginRight: 4 }} />
                      {msg.content}
                    </motion.div>
                  );
                }
            ...
            <div style={{
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <textarea
                  ...
                  placeholder={
                    hasContext
                      ? "Ask about your analysis… (Enter to send)"
                      : "Load context first, then ask questions…"
                  }
                  rows={1}
                  style={{
                    flex:         1,
              ...
              <p style={{
                ...
                Powered by FAISS + Groq API · Answers grounded in your ML pipeline results
              </p>
            </div>
          </motion.div>
        )}
