...
const sections = [
  ...
  { title: "3. Data Storage & Security", content: "Data is stored securely using Firebase (Google Cloud). All files are associated with your Firebase UID and inaccessible to other users. ML model .pkl files are saved per-project under a unique project ID." },
  ...
  { title: "5. Project Data", content: "Each project's CSV files, engineered datasets, trained .pkl files, KPI summaries, and agent results are stored under your project ID. Duplicate datasets are detected by SHA-256 hash." },
      ...
      {hovered && (
        <>
          <motion.div className="absolute inset-0 rounded-2xl"
            style={{ background: "radial-gradient(circle at 0% 50%, rgba(124,92,255,0.1), transparent 65%)" }}
          ...
          <motion.div className="absolute left-0 top-4 bottom-4 w-0.5 rounded-full"
            style={{ background: "linear-gradient(180deg,transparent,#7C5CFF,transparent)" }}
            initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} transition={{ duration: 0.3 }} />
        </>
      )}
      <div className="relative z-10">
        <motion.h2 className="text-white font-bold text-base mb-3 flex items-center gap-2"
          animate={hovered ? { x: 3 } : { x: 0 }} transition={{ duration: 0.2 }}>
          <motion.span className="text-[10px] font-mono px-2 py-0.5 rounded-full"
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <motion.span className="text-xs font-mono text-[#7C5CFF] tracking-widest uppercase block mb-1"
                animate={{ opacity: [0.6, 1, 0.6] }} transition={{ duration: 2, repeat: Infinity }}>Legal</motion.span>
              <h1 className="text-3xl md:text-4xl font-black text-white leading-none">Privacy Policy</h1>
            </div>
          </div>

          <motion.div className="flex items-center gap-3 mb-8 p-4 rounded-2xl relative overflow-hidden"
            ...
            <motion.div className="w-2 h-2 rounded-full bg-[#22C55E]"
              animate={{ scale: [1, 1.5, 1], boxShadow: ["0 0 0 0 #22C55E40", "0 0 0 8px #22C55E00"] }}
